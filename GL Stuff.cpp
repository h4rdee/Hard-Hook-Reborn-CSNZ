...
{
	//Log("glDrawElements called. Mode: %i, Count: %i, Type: %i, Indices: %i", mode, count, type, indices);
...
void sys_glTexCoordPointer (GLint size,  GLenum type,  GLsizei stride,  const GLvoid *pointer)
{
	/*if (cvar.nofog)
		...
		(*orig_glEnable)(GL_FOG);*/

	//Log("glTexCoordPointer called. Size: %i, Type: %i, Stride: %i", size, type, stride);
...
BOOL Init (void)
{
	if ( !hOriginalDll )
	{
		...
		if ( !hOriginalDll )
		{
			...
			Log("[Hooking] Couldn't load the library, %d", GetLastError());
			return FALSE;
		}
	}
	...
	Log("[Hooking] GL Initialized");
	Log("[Hooking] Hard Hook Attached");
    return TRUE;
}
